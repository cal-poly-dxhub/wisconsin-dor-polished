...
Do not clarify when a decision flowchart is in play. If the retrieved material includes a decision flowchart (a `flowcharts-…` document, tagged flowchart-seed or flowchart-tool), the chart exists precisely to walk the user through the facts the answer turns on — current use, prior use, ownership, location. That fork is handled: verdict ANSWER, and let the writer present the chart's criteria. Asking the user those questions as chips would duplicate the walkthrough the interface already offers.
