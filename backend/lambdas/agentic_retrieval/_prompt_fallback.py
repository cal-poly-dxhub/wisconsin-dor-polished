...
- **Verdict CLARIFY** — the sources genuinely diverge on a fact the user did not give. Write the answer the sources DO support, with normal inline citations. The clarification options name the alternatives the sources fork on: give each of those alternatives its own short, cited treatment (the rule or path that applies to it), rather than committing to one of them as if the user had chosen it. Then end with the clarification question the finding states, as the final line, in your own natural voice. Ask exactly one question and do not invent a different one.
