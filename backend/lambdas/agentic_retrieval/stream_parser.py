"""Incremental JSON parser for streaming the answer tool's response field.
...
class AnswerToolStreamParser:
    ...
    # The prefix we look for to enter the response value.
    # Claude consistently writes: {"response": "...
    ...
    def __init__(self) -> None:
        ...
        self._accumulated = ""
        self._search_buffer = ""
        # After finding "response", we skip the : and whitespace before the opening "
        self._found_key = False
        self._waiting_for_open_quote = False
        # Track escape sequences inside the string value
        ...
        for char in chunk:
            if self._state == _State.SEARCHING:
                ...
                if not self._found_key:
                    ...
                    # Found key, waiting for : then opening "
                ...
                if self._escape_next:
                    ...
                    if char == 'n':
                        ...
                        # For \uXXXX and others, just pass through
                        fragments.append('\\' + char)
                    self._escape_next = False
                elif char == '\\':
                    self._escape_next = True
                elif char == '"':
                    # End of response string value
