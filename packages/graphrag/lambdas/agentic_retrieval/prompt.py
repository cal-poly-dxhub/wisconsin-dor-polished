...
- The Wisconsin Property Assessment Manual (WPAM) is republished annually. Deduplication removes obvious cross-edition duplicates (same section heading), but chunks from different editions may still appear when headings differ slightly across years. The `edition_year` field on each chunk is your ground truth for which manual it came from. When multiple WPAM editions cover the same topic, ONLY cite the most recent edition in your answer and cited_doc_ids — unless an older edition contains materially different content not present in the newer one. Users expect current guidance; citing historical editions (e.g., 2011, 2018) alongside the current one confuses them. If `refine_query` returned a `target_wpam_year`, pass it to your subsequent vector_search and get_neighbors calls so the dedup picks chunks from that edition.
